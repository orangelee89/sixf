...
def print_once(msg: str):
    global _printed_once_set
    if msg not in _printed_once_set:
        print(msg)
        _printed_once_set.add(msg)
...
def compute_sixfeet_rewards_directional(
    ...
    num_self_collisions_per_env: torch.Tensor, cfg_rew_scale_self_collision: float,
    was_severely_tilted_last_step: torch.Tensor, cfg_rew_scale_successful_flip: float,
    cfg_target_height_reward_sharpness: float,
    foot_quats_w: Optional[torch.Tensor], foot_contact_mask: Optional[torch.Tensor],
    ...
    num_feet_in_contact: torch.Tensor,
    cfg_rew_scale_all_feet_stable_stand: float,
    cfg_rew_scale_airborne_feet_penalty: float,
    target_standing_joint_angles_tensor: Optional[torch.Tensor],
    imitation_relevant_dof_indices: Optional[torch.Tensor],
    cfg_rew_scale_standing_pose_imitation: float,
    cfg_standing_pose_imitation_sharpness: float
    ...
    height_threshold_for_vel_penalties = cfg_target_height_m - 0.02
    ...
    dof_range = q_upper_limits - q_lower_limits; dof_range = torch.where(dof_range < 1e-6, torch.ones_like(dof_range), dof_range)
    ...
    near_lower_limit = torch.relu(threshold_percent - dof_pos_scaled_01)**2; near_upper_limit = torch.relu(dof_pos_scaled_01 - (1.0 - threshold_percent))**2
    ...
    is_too_low = (current_height_z < cfg_min_height_penalty_threshold).float(); penalty_low_height = is_too_low * cfg_rew_scale_low_height_penalty
    ...
    cos_angle_robot_z_with_world_z_dev = -projected_gravity_b[:, 2]; angle_deviation_from_world_z = torch.acos(torch.clamp(cos_angle_robot_z_with_world_z_dev, -1.0 + 1e-7, 1.0 - 1e-7)); penalty_orientation_deviation = cfg_rew_scale_orientation_deviation * angle_deviation_from_world_z
    penalty_self_collision = cfg_rew_scale_self_collision * (num_self_collisions_per_env > 0).float()
    current_is_severely_tilted_for_flip = projected_gravity_b[:, 2] > 0.0; transitioned_to_upright_hemisphere = was_severely_tilted_last_step & (~current_is_severely_tilted_for_flip); reward_successful_flip = cfg_rew_scale_successful_flip * transitioned_to_upright_hemisphere.float()
    
    reward_custom_foot_axis_alignment = torch.zeros_like(root_pos_w[:, 0], device=device)
    if foot_quats_w is not None and foot_contact_mask is not None and foot_link_target_align_local_axes is not None and foot_link_target_align_world_axes is not None and cfg_rew_scale_custom_foot_axis_alignment != 0.0:
        num_feet_to_check = foot_quats_w.shape[1]
        if num_feet_to_check > 0 and foot_link_target_align_local_axes.shape[0] == num_feet_to_check and foot_link_target_align_world_axes.shape[0] == num_feet_to_check:
            all_feet_alignment_quality = torch.zeros_like(foot_contact_mask, dtype=torch.float32)
            for i in range(num_feet_to_check):
                current_foot_quats_w_single_foot = foot_quats_w[:, i, :]; current_local_axis_to_align = foot_link_target_align_local_axes[i, :].expand(current_foot_quats_w_single_foot.shape[0], 3); current_target_world_axis = foot_link_target_align_world_axes[i, :].expand(current_foot_quats_w_single_foot.shape[0], 3)
                foot_local_axis_in_world = quat_rotate(current_foot_quats_w_single_foot, current_local_axis_to_align)
                cos_sim = torch.sum(foot_local_axis_in_world * current_target_world_axis, dim=1); alignment_quality = (cos_sim + 1.0) / 2.0; all_feet_alignment_quality[:, i] = alignment_quality
            active_foot_alignment_reward = all_feet_alignment_quality * foot_contact_mask.float(); num_contacting_feet_for_align = torch.sum(foot_contact_mask.float(), dim=1); sum_alignment_reward_per_env = torch.sum(active_foot_alignment_reward, dim=1)
            reward_custom_foot_axis_alignment_values = sum_alignment_reward_per_env / torch.clamp(num_contacting_feet_for_align, min=1.0); reward_custom_foot_axis_alignment = torch.where(num_contacting_feet_for_align > 0, reward_custom_foot_axis_alignment_values * cfg_rew_scale_custom_foot_axis_alignment, torch.zeros_like(sum_alignment_reward_per_env))

    reward_all_feet_stable_stand = torch.zeros_like(root_pos_w[:, 0], device=device); penalty_airborne_feet = torch.zeros_like(root_pos_w[:, 0], device=device); num_total_feet_const = 6
    stand_and_upright_condition = is_stand_cmd_active & is_within_orientation_limit
    if cfg_rew_scale_all_feet_stable_stand != 0.0:
        all_feet_contacting = (num_feet_in_contact == num_total_feet_const); all_feet_stable_stand_active = stand_and_upright_condition & all_feet_contacting
        reward_all_feet_stable_stand = torch.where(all_feet_stable_stand_active,torch.full_like(reward_all_feet_stable_stand, cfg_rew_scale_all_feet_stable_stand),torch.zeros_like(reward_all_feet_stable_stand))
    if cfg_rew_scale_airborne_feet_penalty != 0.0:
        airborne_feet_count = num_total_feet_const - num_feet_in_contact; airborne_feet_count_float = torch.relu(airborne_feet_count.float()); _base_penalty_airborne_feet = cfg_rew_scale_airborne_feet_penalty * airborne_feet_count_float
        penalty_airborne_feet = torch.where(stand_and_upright_condition,_base_penalty_airborne_feet,torch.zeros_like(_base_penalty_airborne_feet))

    # --- 姿态模仿奖励 (仅考虑 imitation_relevant_dof_indices) ---
    reward_standing_pose_imitation = torch.zeros_like(root_pos_w[:, 0], device=device)
    if target_standing_joint_angles_tensor is not None and \
       imitation_relevant_dof_indices is not None and \
       imitation_relevant_dof_indices.numel() > 0 and \
       cfg_rew_scale_standing_pose_imitation != 0.0:
        
        # 生效条件：仅当机器人姿态"正面朝上"时 (移除了 is_stand_cmd_active 条件)
        pose_imitation_active_condition = is_within_orientation_limit
        
        current_relevant_joints = current_joint_pos_abs[:, imitation_relevant_dof_indices]
        target_relevant_joints = target_standing_joint_angles_tensor[imitation_relevant_dof_indices]
        
        pose_error_sq = torch.sum(torch.square(current_relevant_joints - target_relevant_joints.unsqueeze(0)), dim=1)
        
        num_relevant_dofs_float = float(imitation_relevant_dof_indices.numel())
        num_relevant_dofs_tensor = torch.tensor(num_relevant_dofs_float, device=pose_error_sq.device, dtype=pose_error_sq.dtype)
        normalized_pose_error_sq = pose_error_sq / torch.clamp(num_relevant_dofs_tensor, min=1.0)

        _base_reward_pose_imitation = torch.exp(-cfg_standing_pose_imitation_sharpness * normalized_pose_error_sq) * cfg_rew_scale_standing_pose_imitation
        
        reward_standing_pose_imitation = torch.where(
            pose_imitation_active_condition,
            _base_reward_pose_imitation,
            torch.zeros_like(_base_reward_pose_imitation)
        )
    ...
    total_reward = (
        ...
        + reward_custom_foot_axis_alignment + reward_all_feet_stable_stand + penalty_airborne_feet
        + reward_standing_pose_imitation
        + (penalty_action_cost + penalty_action_rate + penalty_joint_torques + penalty_joint_accel
        + penalty_flat_orientation + penalty_unwanted_movement
        + penalty_dof_at_limit + penalty_toe_orientation + penalty_low_height
        + penalty_undesired_contact) * dt
    )
    total_reward = total_reward + penalty_lin_vel_z + penalty_ang_vel_xy
    
    reward_terms: Dict[str, torch.Tensor] = {
        "move_in_commanded_direction": reward_move_in_commanded_direction, "turn_reward_combined": reward_turn, "alive": reward_alive, "target_height": reward_target_height,
        "orientation_deviation_penalty": penalty_orientation_deviation, "self_collision_penalty": penalty_self_collision,
        "successful_flip_reward": reward_successful_flip, "custom_foot_axis_alignment": reward_custom_foot_axis_alignment,
        "all_feet_stable_stand_reward": reward_all_feet_stable_stand, "airborne_feet_penalty": penalty_airborne_feet,
        "standing_pose_imitation_reward": reward_standing_pose_imitation,
        "action_cost_penalty": penalty_action_cost * dt, "action_rate_penalty": penalty_action_rate * dt,
        "joint_torques_penalty": penalty_joint_torques * dt, "joint_accel_penalty": penalty_joint_accel * dt,
        "lin_vel_z_penalty": penalty_lin_vel_z, "ang_vel_xy_penalty": penalty_ang_vel_xy,
        "flat_orientation_penalty": penalty_flat_orientation * dt, "unwanted_movement_penalty": penalty_unwanted_movement * dt,
        "dof_at_limit_penalty": penalty_dof_at_limit * dt, "toe_orientation_penalty": penalty_toe_orientation * dt,
        "low_height_penalty": penalty_low_height * dt, "undesired_contact_penalty": penalty_undesired_contact * dt,
    }
...
class SixfeetEnv(DirectRLEnv):
    ...
    _foot_link_target_align_local_axes_tensor: Optional[torch.Tensor] = None
    _foot_link_target_align_world_axes_tensor: Optional[torch.Tensor] = None
    _target_standing_joint_angles_tensor: Optional[torch.Tensor] = None
    _j1_dof_indices_list: List[int]
    _j2_dof_indices_list: List[int]
    _j3_dof_indices_list: List[int]
    _imitation_relevant_dof_indices: Optional[torch.Tensor] = None
    ...
    def __init__(self, cfg: SixfeetEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        joint_limits = self.robot.data.joint_pos_limits[0].to(self.device); self._q_lower_limits = joint_limits[:, 0]; self._q_upper_limits = joint_limits[:, 1]
        
        # --- 初始化目标站立姿态张量 和 J1, J2, J3 的DOF索引 ---
        self._target_standing_joint_angles_tensor = None
        self._j1_dof_indices_list = []
        self._j2_dof_indices_list = []
        self._j3_dof_indices_list = []
        self._imitation_relevant_dof_indices = None

        if hasattr(self.cfg, "target_standing_joint_angles_dict") and self.cfg.target_standing_joint_angles_dict:
            if hasattr(self.robot, "dof_names") and self.robot.dof_names:
                num_dof_val = self._q_lower_limits.numel()
                ordered_target_angles_list = [0.0] * num_dof_val
                all_dof_names_from_robot = self.robot.dof_names
                cfg_joint_dict = self.cfg.target_standing_joint_angles_dict
                
                for i in range(num_dof_val):
                    dof_name_from_robot = all_dof_names_from_robot[i]
                    target_angle_for_dof = 0.0; key_found = False; mapped_key_for_type = ""
                    if dof_name_from_robot in cfg_joint_dict:
                        target_angle_for_dof = cfg_joint_dict[dof_name_from_robot]; key_found = True; mapped_key_for_type = dof_name_from_robot
                    else:
                        simplified_name = dof_name_from_robot.replace("_joint", "")
                        if simplified_name in cfg_joint_dict: target_angle_for_dof = cfg_joint_dict[simplified_name]; key_found = True; mapped_key_for_type = simplified_name
                        else:
                            leg_index_1_based = (i // 3) + 1; joint_in_leg_index_1_based = (i % 3) + 1
                            constructed_key = f"joint_{leg_index_1_based}{joint_in_leg_index_1_based}"
                            if constructed_key in cfg_joint_dict: target_angle_for_dof = cfg_joint_dict[constructed_key]; key_found = True; mapped_key_for_type = constructed_key
                    if not key_found: print_once(f"[WARNING] Pose: DOF '{dof_name_from_robot}' not in target_dict. Defaulting to 0.0.")
                    ordered_target_angles_list[i] = target_angle_for_dof
                    key_to_check_type = mapped_key_for_type if mapped_key_for_type else dof_name_from_robot
                    if key_to_check_type.endswith("1"): self._j1_dof_indices_list.append(i)
                    elif key_to_check_type.endswith("2"): self._j2_dof_indices_list.append(i)
                    elif key_to_check_type.endswith("3"): self._j3_dof_indices_list.append(i)
                    elif "_coxa" in dof_name_from_robot: self._j1_dof_indices_list.append(i)
                    elif "_femur" in dof_name_from_robot: self._j2_dof_indices_list.append(i)
                    elif "_tibia" in dof_name_from_robot: self._j3_dof_indices_list.append(i)
                if len(ordered_target_angles_list) == num_dof_val: self._target_standing_joint_angles_tensor = torch.tensor(ordered_target_angles_list, device=self.device, dtype=torch.float32); print(f"[INFO] Target standing joint angles tensor initialized.")
                if self._j2_dof_indices_list and self._j3_dof_indices_list:
                    j2_indices = torch.tensor(sorted(list(set(self._j2_dof_indices_list))), device=self.device, dtype=torch.long)
                    j3_indices = torch.tensor(sorted(list(set(self._j3_dof_indices_list))), device=self.device, dtype=torch.long)
                    if j2_indices.numel() > 0 or j3_indices.numel() > 0 : self._imitation_relevant_dof_indices = torch.cat([t for t in (j2_indices, j3_indices) if t.numel() > 0]); print_once(f"[INFO] Relevant DOF indices for pose imitation (J2+J3): {self._imitation_relevant_dof_indices.tolist()}")
                    else: print_once("[WARNING] No J2 or J3 DoFs for pose imitation.")
                else: print_once("[WARNING] J2 and/or J3 DoFs not identified. Pose imitation for specific joints disabled.")
            else: print_once("[WARNING] robot.dof_names not available.")
        else: print_once("[INFO] 'target_standing_joint_angles_dict' not in Cfg. Pose imitation disabled.")

        # --- 初始化用于自定义足部姿态对齐的连杆索引和目标轴 ---
        self._foot_link_articulation_indices = []; self._foot_link_sensor_indices = []; self._foot_link_names_found_for_align = []
        target_align_axes_list = []; target_world_axes_list = []
        world_z_axis = torch.tensor([0.0,0.0,1.0],device=self.device,dtype=torch.float32); local_x_axis=torch.tensor([1.0,0.0,0.0],device=self.device,dtype=torch.float32); local_neg_x_axis=torch.tensor([-1.0,0.0,0.0],device=self.device,dtype=torch.float32); local_y_axis=torch.tensor([0.0,1.0,0.0],device=self.device,dtype=torch.float32); local_neg_y_axis=torch.tensor([0.0,-1.0,0.0],device=self.device,dtype=torch.float32); local_z_axis_default=torch.tensor([0.0,0.0,1.0],device=self.device,dtype=torch.float32)
        if hasattr(self.cfg, "foot_link_name_pattern_for_custom_align") and self.cfg.foot_link_name_pattern_for_custom_align:
            ...
            if foot_pattern_re and hasattr(self.robot,"body_names")and self.robot.body_names is not None:
                ...
                for i,body_name in enumerate(self.robot.body_names):
                    if foot_pattern_re.fullmatch(body_name):temp_foot_infos.append((i,body_name))
                temp_foot_infos.sort(key=lambda x:x[1])
                for artic_idx,body_name in temp_foot_infos:
                    self._foot_link_articulation_indices.append(artic_idx);self._foot_link_names_found_for_align.append(body_name)
                    if body_name.endswith("33")or body_name.endswith("63"):target_align_axes_list.append(local_y_axis)
                    elif body_name.endswith("53"):target_align_axes_list.append(local_neg_x_axis)
                    elif body_name.endswith("23"):target_align_axes_list.append(local_x_axis)
                    elif body_name.endswith("43")or body_name.endswith("13"):target_align_axes_list.append(local_neg_y_axis)
                    else:target_align_axes_list.append(local_z_axis_default)
                    target_world_axes_list.append(world_z_axis)
                if self._foot_link_names_found_for_align:
                    ...
                    if sensor_indices_found and len(sensor_indices_found)==len(self._foot_link_names_found_for_align):
                        ...
                        if target_align_axes_list:self._foot_link_target_align_local_axes_tensor=torch.stack(target_align_axes_list);self._foot_link_target_align_world_axes_tensor=torch.stack(target_world_axes_list)
                        print(f"[INFO] Articulation indices for custom align: {self._foot_link_articulation_indices}");print(f"[INFO] Sensor body indices for custom align/contact: {self._foot_link_sensor_indices} for names {self._foot_link_names_found_for_align}")
                    else:self._foot_link_articulation_indices=[];self._foot_link_sensor_indices=[];self._foot_link_names_found_for_align=[];self._foot_link_target_align_local_axes_tensor=None;self._foot_link_target_align_world_axes_tensor=None;print_once(f"[WARNING] Could not map all foot link names to sensor indices consistently for custom alignment.")
                else:print_once(f"[WARNING] No robot body names matched foot pattern for custom alignment: {foot_pattern_str}")
        if not self._foot_link_articulation_indices or len(self._foot_link_articulation_indices)!=len(target_align_axes_list)or not self._foot_link_sensor_indices or len(self._foot_link_sensor_indices)!=len(self._foot_link_articulation_indices)or self._foot_link_target_align_local_axes_tensor is None:
            if hasattr(self.cfg,"rew_scale_custom_foot_axis_alignment")and getattr(self.cfg,"rew_scale_custom_foot_axis_alignment",0.0)!=0.0 or hasattr(self.cfg,"rew_scale_all_feet_stable_stand")and getattr(self.cfg,"rew_scale_all_feet_stable_stand",0.0)!=0.0 or hasattr(self.cfg,"rew_scale_airborne_feet_penalty")and getattr(self.cfg,"rew_scale_airborne_feet_penalty",0.0)!=0.0:print_once(f"[WARNING] Rewards related to foot alignment/contact may be disabled due to setup issues in __init__.")
            self._foot_link_articulation_indices=[];self._foot_link_sensor_indices=[];self._foot_link_names_found_for_align=[];self._foot_link_target_align_local_axes_tensor=None;self._foot_link_target_align_world_axes_tensor=None
        
        self._policy_actions = torch.zeros(self.num_envs, self.cfg.action_space, device=self.device); self._previous_policy_actions = torch.zeros_like(self._policy_actions); self._processed_actions = torch.zeros_like(self._policy_actions)
        self._commands = torch.zeros(self.num_envs, 3, device=self.device, dtype=torch.float); self._time_since_last_command_change = torch.zeros(self.num_envs, device=self.device)
        self._resolve_toe_joint_indices()
        self._undesired_contact_body_ids: Optional[List[int]] = None
        if hasattr(self.cfg, "undesired_contact_link_names_expr") and self.cfg.undesired_contact_link_names_expr and hasattr(self.cfg, "rew_scale_undesired_contact") and self.cfg.rew_scale_undesired_contact != 0.0:
        ...
        if hasattr(self.cfg, "termination_base_contact") and self.cfg.termination_base_contact and hasattr(self.cfg, "base_link_name") and self.cfg.base_link_name:
    ...
    def _get_rewards(self) -> torch.Tensor:
        # ... (确保所有状态变量的获取与上一版本一致) ...
        root_lin_vel_b=self.robot.data.root_lin_vel_b; root_ang_vel_b=self.robot.data.root_ang_vel_b; projected_gravity_b=self.robot.data.projected_gravity_b; default_pos_expanded=self._default_joint_pos.unsqueeze(0)if self._default_joint_pos.ndim==1 else self._default_joint_pos; joint_pos_rel=self.robot.data.joint_pos-default_pos_expanded; current_joint_pos_abs=self.robot.data.joint_pos; joint_vel=self.robot.data.joint_vel; applied_torque=self.robot.data.applied_torque; joint_acc=getattr(self.robot.data,"joint_acc",torch.zeros_like(joint_vel,device=self.device)); root_pos_w=self.robot.data.root_pos_w
        undesired_contacts_active=torch.zeros(self.num_envs,device=self.device,dtype=torch.bool)
        if hasattr(self.cfg,"rew_scale_undesired_contact")and self.cfg.rew_scale_undesired_contact!=0.0 and self._undesired_contact_body_ids and len(self._undesired_contact_body_ids)>0:
            if hasattr(self._contact_sensor.data,'net_forces_w_history')and self._contact_sensor.data.net_forces_w_history is not None:
                ...
                if all_forces_contact_sensor.ndim==4 and all_forces_contact_sensor.shape[1]>0 and self._undesired_contact_body_ids and max(self._undesired_contact_body_ids)<all_forces_contact_sensor.shape[2]: forces_undesired=all_forces_contact_sensor[:,-1,self._undesired_contact_body_ids,:];undesired_contacts_active=torch.any(torch.norm(forces_undesired,dim=-1)>1.0,dim=1)
        ...
        if hasattr(contact_data,'body_indices_in_contact_buffer')and contact_data.body_indices_in_contact_buffer is not None and hasattr(self.robot.data,'body_indices')and self.robot.data.body_indices is not None:
            ...
            if robot_global_body_indices.numel()>0 and contact_pairs_global_indices.numel()>0:
                ...
                if num_envs_contact==self.num_envs and num_envs_robot==self.num_envs:
                    ...
                    if hasattr(contact_data,'num_contacts_in_buffer')and contact_data.num_contacts_in_buffer is not None: valid_contact_mask_sensor=torch.arange(max_contacts_per_env,device=self.device).unsqueeze(0)<contact_data.num_contacts_in_buffer.unsqueeze(1);is_self_collision_pair_valid=is_self_collision_pair&valid_contact_mask_sensor&valid_contact_indices
        ...
        foot_quats_w_for_reward:Optional[torch.Tensor]=None; foot_contact_mask_for_custom_align:Optional[torch.Tensor]=None; num_feet_in_contact=torch.zeros(self.num_envs,device=self.device,dtype=torch.long)
        if self._foot_link_articulation_indices and self._foot_link_sensor_indices and len(self._foot_link_articulation_indices)>0 and len(self._foot_link_articulation_indices)==len(self._foot_link_sensor_indices):
            if hasattr(self.robot.data,"body_quat_w")and self.robot.data.body_quat_w is not None:
                ...
                if articulation_indices_tensor.numel()>0 and articulation_indices_tensor.max()<self.robot.data.body_quat_w.shape[1]: foot_quats_w_for_reward=self.robot.data.body_quat_w[:,articulation_indices_tensor]
            if hasattr(self._contact_sensor.data,'net_forces_w_history')and self._contact_sensor.data.net_forces_w_history is not None:
                all_forces_contact=self._contact_sensor.data.net_forces_w_history;sensor_indices_tensor=torch.tensor(self._foot_link_sensor_indices,device=self.device,dtype=torch.long)
                if sensor_indices_tensor.numel()>0 and all_forces_contact.ndim==4 and all_forces_contact.shape[1]>0 and sensor_indices_tensor.max()<all_forces_contact.shape[2]:
                    forces_on_these_feet=all_forces_contact[:,-1,sensor_indices_tensor,:];foot_contact_magnitudes=torch.norm(forces_on_these_feet,dim=-1)
                    foot_contact_mask_for_custom_align=foot_contact_magnitudes>1.0;num_feet_in_contact=torch.sum(foot_contact_mask_for_custom_align.long(),dim=1)
        ...
        total_reward, reward_terms_dict = compute_sixfeet_rewards_directional(
            ...
            self.cfg.command_profile, # 传递原始的，确保CFG中已修正类型
            ...
            foot_quats_w_for_reward,
            foot_contact_mask_for_custom_align,
            self._foot_link_target_align_local_axes_tensor,
            self._foot_link_target_align_world_axes_tensor,
            getattr(self.cfg, "rew_scale_custom_foot_axis_alignment", 0.0),
            num_feet_in_contact,
            getattr(self.cfg, "rew_scale_all_feet_stable_stand", 0.0),
            getattr(self.cfg, "rew_scale_airborne_feet_penalty", 0.0),
            self._target_standing_joint_angles_tensor,
            self._imitation_relevant_dof_indices, # <<< 确保这个参数被传递
            getattr(self.cfg, "rew_scale_standing_pose_imitation", 0.0),
            getattr(self.cfg, "standing_pose_imitation_sharpness", 1.0)
        )
